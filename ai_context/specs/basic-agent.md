...
| **Env**      | `**AI_GATEWAY_API_KEY`** in `.env.local` locally; create a key in the Vercel dashboard ([AI Gateway API Keys](https://vercel.com/docs/ai-gateway)). For production on Vercel, prefer dashboard env vars or **OIDC**-based Gateway auth where applicable.                                      |
